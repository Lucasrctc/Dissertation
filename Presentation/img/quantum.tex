...
\begin{tikzpicture}[thick]
	...
	%
	\matrix[row sep=0.1cm, column sep=0.5cm] (circuit) {
	% First row.
		\node[bit] (x00) {$\ket {0}_0$}; &
			\coordinate (comp00);&&&
		\coordinate (comp01);&
		\coordinate (x02);&
		\coordinate (y0);
	...
	\begin{pgfonlayer}{background}
		...
		% Draw lines.
		\draw[thick] (x00) -- (comp00) ;
		\draw[thick] (x10) -- (comp10) ;
		\draw[thick] (xn0) -- (compn0) ;
		...
		\node[computer] (computer) [fit = (comp00) (xn2)] {};
		\node at (computer) {$U$};
		\draw[thick] (computer.east) -- ([xshift=1em]computer.east) ;
		\node[bit] at ([xshift=2em]computer.east) (psi) {$\ket {\psi}$}; 
	\end{pgfonlayer}
%
\end{tikzpicture}
\end{document}
